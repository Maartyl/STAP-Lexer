...
void lex(State s){
	...
	if (st_getType(s) == stt_NONE) return; //has no state, nothing to flush...
	st_setChar(s, -1); //fake value, can't match anything new
	st_updateFnp(s); ///doesn't really work: in str, ...
	st_Fn(s);
	st_Fn2(s);
}
